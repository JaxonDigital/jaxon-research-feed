#!/usr/bin/env python3
"""
Generate podcast RSS feed for Jaxon Research articles.

Usage:
    python3 generate-feed.py

Reads MP3 files from /Users/bgerby/Documents/dev/ai/audio-reviews/
...
def get_mp3_metadata(mp3_path):
    ...
    try:
        ...
        return {
            ...
            'album': tags.get('album', 'Jaxon Research'),
...
def parse_filename(filename):
    """
    Parse filename to extract episode info.
    Expected format: GAT-321.mp3 or 01-321-article-title.mp3
    Returns (episode_number, ticket_number, title_slug)
    """
    stem = Path(filename).stem

    # Try new format: 01-321-article-title
    if stem[0].isdigit() and '-' in stem:
        parts = stem.split('-', 2)
        if len(parts) >= 2:
            seq = parts[0]
            ticket = parts[1]
            title_slug = parts[2] if len(parts) > 2 else ''
            return (seq, ticket, title_slug)

    # Try old format: GAT-321
    if stem.startswith('GAT-'):
        ticket = stem.replace('GAT-', '')
        return (None, ticket, '')
...
def generate_rss(output_path):
    ...
    # Channel metadata
    SubElement(channel, 'title').text = 'Jaxon Research Feed'
    SubElement(channel, 'description').text = (
        'Audio reviews of high-priority articles relevant to Jaxon Digital\'s '
        'AI agent and DXP platform initiatives. Includes strategic analysis, '
    ...
    itunes_image.set('href', 'https://jaxondigital.github.io/jaxon-research-feed/cover-art.png')
    ...
    # Find all MP3 files
    mp3_files = sorted(AUDIO_DIR.glob('GAT-*.mp3'))
    ...
    for mp3_path in mp3_files:
        ...
        seq, ticket, title_slug = parse_filename(mp3_path.name)
        ...
        episodes.append({
            ...
            'description': metadata['comment'] or f"Article review - {metadata['title']}",
            'audio_url': audio_url,
            'pub_date': pub_date,
    ...
    # Sort episodes by sequence number if available, else by ticket number
    episodes.sort(key=lambda e: (e['seq'] or '999', e['ticket'] or '999'), reverse=True)
